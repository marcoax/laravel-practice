...
changes that affect _how_ you build — new features, security, breaking changes.
...
- Each lesson is a _brief_: what changed, why it might matter, what to try, and a
...
1. **Discover** new releases from the editorial sources listed in `learning-config.md`
   ...
   querying them all. A release is lesson-worthy if _any_ of the sources wrote about it.
...
finish a lesson** — a read-only check that only _proposes_ new lessons, never generates them
...
writes status and notes at the end of each lesson (the _lesson lifecycle gate_), and
...
2. _Settings → Pages → Source: Deploy from a branch → `main` / root_.
